...
router.put("/register_user", (req, res) => {
  try {
    ...
    var main_db = new PouchDB(
      `https://${process.env.host}/database/data`
    );

    new Promise((resolve, reject) => {
      var users_db = new PouchDB(
        `https://${process.env.host}/database/users`
      );
      ...
      users_db.info().then(function () {
        users_db
          .put(userData)
          .then(function (result) {
            console.log(`Result of User data Add: ${JSON.stringify(result, null, 2)}`);
          })
          .catch((err) => {
